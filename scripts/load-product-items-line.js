document.addEventListener('DOMContentLoaded', () => {
  ...
  function setupCartPopupEvents() {
    document.querySelectorAll('.add-to-cart-btn').forEach(btn => {
      console.log('Setting up event for button:', btn);
      btn.addEventListener('click', () => {
        ...
        fetch('../components/added-to-cart.html')
          ...
          .then(popupHTML => {
            ...
            // 삽입 전에 placeholder 비워주기
            placeholder.innerHTML = popupHTML;
  
            // DOM에 삽입 완료된 뒤에 애니메이션 트리거
            requestAnimationFrame(() => {
              ...
              closeBtn.addEventListener('click', () => {
                // .active 제거하면 사라지는 애니메이션 실행됨
                overlay.classList.remove('active');
  
                // 애니메이션 끝나고 나서 DOM 제거
                setTimeout(() => {
                  overlay.remove();
                }, 400); // CSS transition-duration 과 일치
              });
            });
          })
